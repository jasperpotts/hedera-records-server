...
export const computeAccountParameters = (configuration) =>
  computeProperties(
    ...
    () => {
      ...
      const accountPath = `${configuration.baseApiUrl}/accounts?account.balance>0&limit=1&account.id=969040&order=desc`;
...
export const setDefaultValuesForEnvParameters = () => {
  __ENV['BASE_URL'] = __ENV['BASE_URL'] || 'http://localhost:8080';
